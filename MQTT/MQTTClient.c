...
static int sendPacket(MQTTClient* c, int length, Timer* timer)
{
    ...
    if (sent == length)
    {
        TimerCountdown(&c->last_sent, c->keepAliveInterval); // record the fact that we have MQTT_SUCCESS fully sent the packet
        rc = MQTT_SUCCESS;
    }
    else
        rc = FAILURE;
...
static int readPacket(MQTTClient* c, Timer* timer)
{
    ...
    if (c->keepAliveInterval > 0)
        TimerCountdown(&c->last_received, c->keepAliveInterval); // record the fact that we have MQTT_SUCCESS fully received a packet
exit:
    return rc;
}

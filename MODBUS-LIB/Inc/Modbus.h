...
typedef struct
{
    uint8_t u8RawDataCnt;
    uint16_t u16RawData[32];
}modbusRecvRawData_t;
/**
 * @enum MESSAGE
 * @brief
...
typedef struct
{
    ...
    // add by ZJ to store origin raw data
    bool is_fc3_processed;
    modbusRecvRawData_t recvRawData;
...
// the following functions added by ZJ

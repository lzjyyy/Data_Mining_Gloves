...
static bool is_fc3_processed = false;
static modbusRecvRawData_t recvRawData;

bool get_is_fc3_processed(modbusHandler_t* modH)
{
	return modH->is_fc3_processed;
}

void reset_is_fc3_processed(modbusHandler_t* modH)
{
	modH->is_fc3_processed = false;
}

void set_is_fc3_processed(modbusHandler_t* modH)
{
	modH->is_fc3_processed = true;
}

modbusRecvRawData_t* get_recv_raw_data_addr(modbusHandler_t* modH)
{
	return &modH->recvRawData;
}
...
static void get_FC6(modbusHandler_t* modH);
static void get_FC16(modbusHandler_t* modH);
...
void vTimerCallbackT35(TimerHandle_t* pxTimer)
{
	...
	for (i = 0; i < numberHandlers; i++)
	{
		...
		if ((TimerHandle_t*)mHandlers[i]->xTimerT35 == pxTimer) {
			// printf("T3.5 triggered, bytes in buffer: %d\n", RingCountBytes(&mHandlers[i]->xBufferRX));
...
void get_FC3(modbusHandler_t* modH)
{
	...
	for (i = 0; i < modH->u8Buffer[2] / 2; i++)
	{
		...
		// printf("0x%x\r\n", (modH->u8Buffer[u8byte] << 8) | modH->u8Buffer[u8byte + 1]);
		// modH->u16regs[i] = (modH->u8Buffer[u8byte] << 8) | modH->u8Buffer[u8byte + 1];
		modH->recvRawData.u16RawData[i] = (modH->u8Buffer[u8byte] << 8) | modH->u8Buffer[u8byte + 1];
		// printf("Reg[%d] = 0x%x\r\n", i, modH->u16regs[i]);
		u8byte += 2;
	}
	set_is_fc3_processed(modH);
	// printf("Processed: ");
	// for (int i = 0; i < modH->u8Buffer[2] / 2; i++) {
	// 	printf("0x%x ", modH->u16regs[i]);
	// }
	// printf("\r\n");
}

/**
 * This method processes functions 5 & 6 (for master)
 * This method puts the slave answer into master data buffer
 *
 * @ingroup register
 */
void get_FC6(modbusHandler_t* modH)
{
	uint8_t u8byte, i;
	u8byte = 3;
...
/**
 * This method processes functions 16 (for master)
 * This method puts the slave answer into master data buffer
 *
 * @ingroup register
 */
void get_FC16(modbusHandler_t* modH)
{
	uint8_t u8byte, i;
	u8byte = 3;

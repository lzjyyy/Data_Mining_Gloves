...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
 System_VectorTable_Init(0x08020000U);
  /* USER CODE END 1 */
...
//float IN1[6]={60.0f,60.0f,60.0f,60.0f,60.0f,60.0f};
//uint8_t OUT1[12]={0};

//uint8_t IN2[12]={0x00,0x3C,0x00,0x3C,0x00,0x3C,0x00,0x3C,0x00,0x3C,0x00,0x3C};
//float OUT2[6]={0.f};

//floats_to_bytes(IN1,OUT1,12); 
//bytes_to_floats(IN2,OUT2,6);
...
//	MX_TIM16_Init();
  ...
  MX_I2C1_Init();
  MX_I2C3_Init();
  StoreTask_Read485ConfigFromEEPROM();
  MX_USART1_UART_Init();
  MX_USART3_UART_Init();
  MX_SPI3_Init();
  MX_FDCAN1_Init();
...
void System_VectorTable_Init(uint32_t vector_table_addr)
{
    ...
    __enable_irq();
		SCB->ICSR |= SCB_ICSR_PENDSTCLR_Msk;  // ������������� SysTick pending
}
/* USER CODE END 4 */

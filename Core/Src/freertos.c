...
static void mqtt_publish_gripper_status(GripperStatus_t* status, uint8_t side);
...
reconnect:
  ...
  data.MQTTVersion = 4;
  data.clientID.cstring = "STM32_Client";
  data.keepAliveInterval = 60;   // 60秒心�?
...
void messageArrived(MessageData* data)
{
  ...
  if (left) {
    ...
    printf("Mqtt msg left gripper info,pos = %d, speed = %d, tprque = %d\r\n",
  ...
  if (right) {
    ...
    printf("Mqtt msg right gripper info,pos = %d, speed = %d, tprque = %d\r\n",
...
void mqtt_publish_gripper_status(GripperStatus_t* status, uint8_t side)
{
  char topic[64];
  char payload[128];

...
class OTAUpgradeDialog(QDialog):
    ...
    def handle_serial_data(self, raw: bytes) -> bool:
        ...
        # 升级请求：应答序列必须严格为两帧固定内容：
        #  1) c8 10 01 00 00 04 04 12 34 56 78 57 F2
        #  2) c8 10 01 00 00 04 04 87 65 34 21 C2 B5
        if self.expect == "upgrade":
            seq1 = b"\xC8\x10\x01\x00\x00\x02\x04\x12\x34\x56\x78\x57\x94"
            seq2 = b"\xC8\x10\x01\x00\x00\x02\x04\x87\x65\x43\x21\xE5\x23"
            # 只要前缀匹配即可视为命中一帧（通常长度正好是 13B）
            if self.upgrade_seq_pos == 0 and raw[:len(seq1)] == seq1:
                self.upgrade_seq_pos = 1
                self.log("✔ 收到升级应答 1/2")
                return True
            if self.upgrade_seq_pos == 1 and raw[:len(seq2)] == seq2:
